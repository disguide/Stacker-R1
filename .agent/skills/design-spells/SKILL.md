...
4. **Implement flawlessly**: Use `react-native-reanimated` and `expo-haptics` to build the specific micro-interaction with silky-smooth performance (60fps+).
5. **Haptic Spells**: Use subtle haptic feedback (e.g., `Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Light)`) when a toggle is flipped or a task is completed to add tactile "magic."
...
- **Mobile Performance**: A broken or janky "spell" is worse than none. Ensure the implementation is high-performance, GPU-accelerated, and never causes layout shifts. Limit active "spells" to one per interaction to save battery.
